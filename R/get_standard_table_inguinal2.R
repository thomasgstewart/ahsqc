...
get_standard_table_inguinal2 <- function(tbl = NULL
                                        , data
                                        , print = FALSE
                                        , overwrite = FALSE
                                        , pval = FALSE){
  ...
  table_list <- list(
    ...
    , dt = data %>% filter(ssi_treatment %in% \"Yes\")
    , fmt = count_fmt
    ) %>%
    binary_entry(
    ...
    , xlab = \"@@Oral antibiotics\"
    , dt = data %>% filter(sso_treatment %in% \"Yes\")
    , fmt = count_fmt
    ) %>%
    binary_entry(
    ...
    , xlab = \"@@Oral antibiotics\"
    , dt = data %>% filter(ssi_sso_treatment %in% \"Yes\")
    , fmt = count_fmt
    ) %>%
    binary_entry(
