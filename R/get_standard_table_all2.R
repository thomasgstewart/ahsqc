...
get_standard_table_all2 <- function(tbl = NULL
  ...
  table_list <- list(
    ...
    cat_entry_all(e_procedure_category
    , pvalue = FALSE
    , xlab = \"Procedure category\"
    ) %>%
    cat_entry_all(recurrent %>% factor(levels = c(\"Primary\", \"Recurrent\"))
    , pvalue = FALSE
    ...
    , "tbl2 <- list() %>%
    n_unique_all(id_surgeon
    , xlab = \"Surgeons contributing data\") %>%
    n_unique_all(id_site
    ...
    , "tbl3 <- list() %>%
    cont_entry_all(val_age_new
    , xlab = \"Age (years; capped at 90)\"
    , pvalue = FALSE) %>%
    binary_entry_all(female
    , pvalue = FALSE
    , xlab = \"Female\") %>%
    cat_entry_all(e_race
    , pvalue = FALSE
    , xlab = \"Race\") %>%
    cont_entry_all(val_calc_bmi2
    , xlab = \"BMI (kg/m<sup>2</sup>; capped at 15, 60)\"
    , pvalue = FALSE) %>%
    cat_entry_all(bmi_cat %>% factor(levels = c(\"< 30\", \">= 30\"))
    , xlab = \"BMI categories\"
    , pvalue = FALSE) %>%
    cat_entry_all(e_asaclass
    , xlab = \"ASA class\"
    , pvalue = FALSE) %>%
    cat_entry_all(wound_class
    , xlab = \"Wound class distribution\"
    , pvalue = FALSE) %>%
    cat_entry_all(vhwg
    , xlab = \"Hernia Grade (VHWG)\"
    , pvalue = FALSE) %>%
    cont_entry_all(val_hern_width
    ...
    , "tbl4 <- list() %>%
    binary_entry_all(any_comorbidities %>% factor(levels = c(\"No\",\"Yes\"))
    ...
    cat_entry_all(e_prior_mesh_excision %>% factor(levels = c(\"Complete\", \"Partial\", \"None\"))
    ...
    , "tbl5 <- list() %>%
    cat_entry_all(operative_time
    , pvalue_fmt = garbage_pvalue
    , pvalue = FALSE
    , xlab = \"Operative time\") %>%
    binary_entry_all(planned_concomitant_procedure %>% factor(levels = c(\"No\", \"Yes\"))
    , xlab = \"Planned concomitant procedure\"
    , pvalue = FALSE) %>%
    binary_entry_all(flg_concomitant_proc %>% factor(levels = c(\"No\", \"Yes\"))
    ...
    binary_entry_all(flg_fascial_closure %>% factor(levels = c(\"No\", \"Yes\"))
    , xlab = \"Fascial closure\"
    , pvalue = FALSE
    ) %>% 
    
    binary_entry_all(flg_myofascial_release %>% factor(levels = c(\"No\", \"Yes\"))
    , xlab = \"Myofascial release\"
    , pvalue = FALSE
    ) %>%
    
    
    binary_entry_all(flg_mesh_used %>% factor(levels = c(\"No\", \"Yes\"))
    , xlab = \"Mesh used\"
    , pvalue = FALSE) %>%
    
    cat_entry_all(e_mesh_type4_1 %>% factor(levels = c(\"Permanent Synthetic\", \"Resorbable Synthetic\", \"Biological tissue-derived\", \"Other/Unknown\"))
    , xlab = \"Mesh type (4)\"
    , pvalue = FALSE
    , dt = data %>% dplyr::filter(flg_mesh_used == \"Yes\")
    ) %>% 
    
    cat_entry_all(e_mesh_location %>% factor(levels = c(\"Inlay\", \"Onlay\", \"Sublay\"))
    , xlab = \"Mesh location\"
    , pvalue = FALSE
    , dt = data %>% dplyr::filter(flg_mesh_used == \"Yes\")
    ) %>%
    ...
    binary_entry_all(convert_to_open %>% factor(levels = c(\"No\",\"Yes\"))
